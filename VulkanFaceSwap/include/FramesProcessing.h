...
const std::string DLIB_FACE_TRACKER_PATH{ "../models/shape_predictor_68_face_landmarks.dat" };
...
public:
	FramesProcessing() :
		m_counter{0},
		m_dlibFaceTracker{ DLIB_FACE_TRACKER_PATH.c_str() },
		...
		m_demoClock{ std::chrono::high_resolution_clock::now() }
	{
		...
		// Image 1 (if no video)
		if (!MODE_VIDEO)
			m_image1rgba = { readImageFromFile(TEXTURES_PATH + "texture2.jpg") };

		// Image 2
		prepareImage(TEXTURES_PATH + "texture.jpg", m_image2rgba, m_landmarksImage2);
		if (MODE_VIDEO)
		{
			for (auto i = 0; i < m_images2rgba.size(); i++)
				prepareImage(TEXTURES_PATH + "textures/" + std::to_string(i+1) +".jpg", m_images2rgba.at(i), m_landmarksImages2.at(i));
		}
...
const auto clockIteration = std::chrono::high_resolution_clock::now();
		...
		if (frameState == FramesProducer::FrameState::OK && !image1rgba.empty())
		{
			...
			const auto offsetX2Limit = image1rgba.cols - 2 * offsetXLimit;
			const auto offsetY2Limit = image1rgba.rows - 2 * offsetYLimit;
			cv::Rect faceEstimatedRect;
			if (m_previousFaceRect1 != dlib::rectangle{})
			{
				// Smart detecting (disabled, not boosting performance)
				//const auto previousW = (int)std::round(m_previousFaceRect1.right() - m_previousFaceRect1.left());
				//const auto previousH = (int)std::round(m_previousFaceRect1.bottom() - m_previousFaceRect1.top());
				//offsetX = { std::max(0, (int)std::round(m_previousFaceRect1.left() - previousW * 0.75f)) };
				//offsetY = { std::max(0, (int)std::round(m_previousFaceRect1.top() - previousH * 0.75f)) };
				//const auto offsetX2 = (int)std::round(offsetX + previousW * (1 + 2 * 0.75f));
				//const auto offsetY2 = (int)std::round(offsetY + previousH * (1 + 2 * 0.75f));
				//const auto outsideRegion = (offsetX < offsetXLimit || offsetY < offsetYLimit || offsetX2 > offsetX2Limit || offsetY2 > offsetY2Limit);
				//if (outsideRegion)
					m_previousFaceRect1 = { dlib::rectangle{} };
				//else
					//faceEstimatedRect = { cv::Rect{ offsetX, offsetY, offsetX2 - offsetX, offsetY2 - offsetY } };
			}
			if (m_previousFaceRect1 == dlib::rectangle{})
			{
				offsetX = { offsetXLimit };
				offsetY = { offsetYLimit };
				faceEstimatedRect = { cv::Rect{ offsetX, offsetY, offsetX2Limit - offsetX, offsetY2Limit - offsetY } };
			}
...
private:
	...
	// For demo
	std::array<cv::Mat, 7> m_images2rgba;
	std::array<Landmarks, 7> m_landmarksImages2;
	int m_counter;
	...
	void changeImage2IfTimePast()
	{
		...
		m_counter = timePastEveryXSeconds % m_images2rgba.size();
		//// Option b - Every time tracker is recovered
		//m_counter = (m_counter +1) % m_images2rgba.size();

		m_image2rgba = { m_images2rgba[m_counter] };
		m_landmarksImage2 = { m_landmarksImages2[m_counter] };
	}
